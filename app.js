(function () {
  ...
  let isVoiceTranscriptProcessing = false;
  let voiceProcessingQueue = Promise.resolve();
  let lastRawVoiceTranscript = "";
...
let conversationSyncRunning = false;
  ...
  async function sendMessage() {
    if (isVoiceTranscriptProcessing) {
      showVoiceStatus(
        "Please wait for the Roman Nepali transcript to finish, then review it before sending.",
        true
      );
  ...
  function isNepaliLanguage(code) {
    return (
      code === "ne" ||
      code === "ne-NP" ||
      String(code).toLowerCase().startsWith("ne-")
    );
  }

  function appendTextToMessageInput(text) {
  ...
  function setVoiceTranscriptProcessing(isProcessing) {
    isVoiceTranscriptProcessing =
      isProcessing;

    elements.sendButton.disabled =
      isProcessing;

    elements.improveButton.disabled =
      isProcessing;
  ...
  async function convertNepaliSpeechToRomanNepali(
    devanagariTranscript
  ) {
    const result =
      await bridge.processVoiceRomanization({
        text: devanagariTranscript,
        recentMessages:
          getRecentConversation(),
        metadata: {
          dialect: "Jhapa",
          outputStyle:
            "natural conversational WhatsApp Roman Nepali",
  ...
  async function processFinalVoiceTranscript(
    transcript
  ) {
    ...
    /*
     * English speech can be placed directly into the
     * editable message box.
     */
    if (!isNepaliLanguage(sourceLanguage)) {
      appendTextToMessageInput(
        cleanTranscript
      );
    ...
    /*
     * Nepali speech recognition normally returns
     * Devanagari. MeChatBridge converts it into natural,
     * conversational Roman Nepali before placing it in
     * the editable message box.
     */
    lastRawVoiceTranscript =
      cleanTranscript;

    setVoiceTranscriptProcessing(true);

    setConnectionStatus(
      "Converting Nepali speech to Roman Nepali…"
    );

    showVoiceStatus(
      "Converting your Nepali speech into editable Jhapa-style Roman Nepali…"
    );

    try {
      const romanNepaliText =
        await convertNepaliSpeechToRomanNepali(
          cleanTranscript
        );

      if (!romanNepaliText) {
        throw new Error(
          "No Roman Nepali transcript was returned."
        );
      }
      ...
      showVoiceStatus(
        "Roman Nepali is ready. Review and edit it before sending."
      );

      setConnectionStatus("Online");
    } catch (error) {
      console.error(
        "Roman Nepali voice conversion error:",
        error
      );

      /*
       * Preserve the recognised speech rather than losing
       * the user's message when conversion is unavailable.
       */
      appendTextToMessageInput(
        cleanTranscript
      );
      ...
      showVoiceStatus(
        "The Nepali speech was captured, but Roman Nepali conversion is unavailable. The original transcript has been kept for editing.",
        true
      );

      setConnectionStatus(
        "Roman Nepali conversion unavailable"
      );
    } finally {
      setVoiceTranscriptProcessing(false);
    }
  }

  function queueFinalVoiceTranscript(
    transcript
  ) {
    voiceProcessingQueue =
      voiceProcessingQueue
        .then(() =>
          processFinalVoiceTranscript(
            transcript
          )
        )
        .catch((error) => {
          console.error(
            "Voice transcript queue error:",
            error
          );
        });

    return voiceProcessingQueue;
  }
      ...
      elements.voiceStatus.textContent =
        interimTranscript
          ...
          : isNepaliLanguage(
            ...
            ? "Listening… Final Nepali speech will be converted to editable Roman Nepali."
...
petal.textContent = "🌸";
  ...
  /*
   * Lightweight diagnostics for development and testing.
   */
  window.MeChatBridgeVoice = Object.freeze({
    get isListening() {
      return isListening;
    },
    get isProcessing() {
      return isVoiceTranscriptProcessing;
    },
    get lastRawTranscript() {
      return lastRawVoiceTranscript;
    }
  });
