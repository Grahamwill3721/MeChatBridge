(function () {
  ...
  class MeChatBridgeClient {
    ...
    async processVoiceRomanization({
      text,
      recentMessages = [],
      metadata = {}
    }) {
      ...
      if (!originalText) {
        throw new Error(
          "The Nepali voice transcript is empty."
        );
      }

      const webhookUrl =
        String(
          this.config.voiceRomanWebhookUrl || ""
        ).trim();

      if (!webhookUrl) {
        throw new Error(
          "The Roman Nepali voice webhook URL has not been configured."
        );
      }
      ...
      const payload = {
        ...
        fromUser: participantProfile.fromUser,
        toUser: participantProfile.toUser,
        timestamp: new Date().toISOString(),
        mode: "voice-to-roman-nepali",
        sourceLanguage: "ne",
        targetLanguage: "ne-Latn",
        originalText,
        recentMessages:
          Array.isArray(recentMessages)
            ? recentMessages.slice(-8)
